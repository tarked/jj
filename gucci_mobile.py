...
def change_username(session_id, device_id, iid, last_username, new_username):
    ...
    if "unique_id" in result :
        if(check_is_changed(last_username, session_id, device_id, iid)):
            return f"[ ! ] {Fore.GREEN}Username change successful."
        else:
            return f"[ :( ] {Fore.RED}Failed to change username: " + str(result)
    else:
        return f"[ :( ] {Fore.RED}Failed to change username: " + str(result)
...
def main():
    ...
    s = f"""
{Fore.GREEN}╭───────────────────────╮
{Fore.RED}│GUCCI TIKTOK TOOL      │
{Fore.GREEN}│Enter Session ID Below.│
{Fore.RED}╰───────────────────────╯
{Fore.GREEN}↓↓↓
"""

    session_id = input(f"{s}")

    last_username = get_profile(session_id, device_id, iid)
    if last_username != "None": #tiktokthailand 
        print(f"\n{Fore.GREEN}Your current TikTok username is: {last_username}")
        new_username = input(f"{Fore.GREEN}Enter the new username you wish to set: ")
    ...
    print(f"\n{Fore.BLUE}discord.gg/guccimane\n{Fore.YELLOW}telegram @harbi")
